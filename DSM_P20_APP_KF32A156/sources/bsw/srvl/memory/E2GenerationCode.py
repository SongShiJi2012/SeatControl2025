...
def WriteOutputCFile():
    with open(OutputCFile, 'w') as TxtWrFile:
        ...
        LineStr += '{\n'
        LineStr += '/*  数据块ID  起始地址     长度   备份ID    		 RAM变量地址              	   读写请求状态          	  读写操作结果              预存储标志 */\n'
        TxtWrFile.write(LineStr)
        ...
        for i in range(len(DataBlockAllocation_StarAddr)):
            ...
            LineStr = f'    {{{index_str},    {addr_str},    {size_str},    {backupid_str},    {ptr_str},    EN_EEPROM_STA_IDLE,    E2promNullOp,    FALSE}},\n'
            TxtWrFile.write(LineStr)
        ...
        LineStr += '/***********************************************************************\n'
        LineStr += '* @function name: Srvl_SetE2DataBlockEepOptReq\n'
        LineStr += '* @description:  设置数据块ID的预写入标志\n'
        LineStr += '* @author: python Script Generation\n'
        LineStr += f'* @{StrNowTime}\n'
        LineStr += '***********************************************************************/\n'
        LineStr += 'void Srvl_SetE2DataBlockPreWriteFlg(INT16U DataId, INT8U PreWriteFlg)\n'
        LineStr += '{\n'
        LineStr += '    if(DataId < Srvl_GetE2DataBlockCfgTableLength())\n'
        LineStr += '    {\n'
        LineStr += '        EepromDataBlockCfgTable[DataId].PreWriteFlg = PreWriteFlg;\n'
        LineStr += '    }\n'
        LineStr += '}\n\n'

        LineStr += '/***********************************************************************\n'
        LineStr += '* @function name: Srvl_GetE2DataBlockEepOptReq\n'
        LineStr += '* @description:  获取数据块ID的预写入标志\n'
        LineStr += '* @author: python Script Generation\n'
        LineStr += f'* @{StrNowTime}\n'
        LineStr += '***********************************************************************/\n'
        LineStr += 'INT8U Srvl_GetE2DataBlockPreWriteFlg(INT16U DataId)\n'
        LineStr += '{\n'
        LineStr += '    INT8U PreWriteFlg = FALSE;\n\n'
        LineStr += '    if(DataId < Srvl_GetE2DataBlockCfgTableLength())\n'
        LineStr += '    {\n'
        LineStr += '        PreWriteFlg = EepromDataBlockCfgTable[DataId].PreWriteFlg;\n'
        LineStr += '    }\n\n'
        LineStr += '    return PreWriteFlg;\n'
        LineStr += '}\n\n'
...
def WriteOutputHFile():
    with open(OutputHFile, 'w') as TxtWrFile:
        header = (
            "/************************************************************************************************\n"
            f"{StrNowTime}\n"
            f"File Name:          {OutputHFile}\n"
            "Author:             python Script Generation\n"
            "Description:        E2数据块配置定义表 - 使用excel表进行管理，不可手动修改\n"
            "/************************************************************************************************/\n"
        )

        TxtWrFile.write(header)
        ...
        LineStr += f'#define    D_NOBACKUP              0xFFFF      /* 无备份 */\n'
        TxtWrFile.write(LineStr)
