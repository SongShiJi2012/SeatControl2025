    ...
    uint32_t msg_id)
{
    ...
    const CAN_Type base = g_flexcanBase[instance];
    volatile Kf32a_Canfd_Reg *ControllerRegPtr = (Kf32a_Canfd_Reg *)Can_m_ControllersInfo[base].BaseAddress;
	...
	/*发送总线空闲或者发送完成   注：TCSTA有置位不成功的现象*/
//	if((ControllerRegPtr->CANFD_CTLR0.B.TCSTA == 0x01) || (ControllerRegPtr->CANFD_CTLR0.B.TXSTA == 0x00))
//	{
//
//		Can_m_FdMailBox_Write(base, mb_idx, tx_info);
//		state = STATUS_SUCCESS;
////			if(Can_m_FdMailBox_Write(base, mb_idx, tx_info) != CAN_OK)
////			{
////				Can_m_FdTransmit(base);
////				state = STATUS_ERROR;
////			}
...
//	else
//	{
//		Can_m_FdTransmit(base);//检查是否仲裁失败，若失败则重新仲裁
//		state = STATUS_ERROR;
//	}
		...
		if(instance == 0)
		{
			for(mb_idx = D_CANFD_MAILBOX_TX_START; mb_idx < D_CANFD_MAILBOX_TX_END; mb_idx++)//从发送邮箱配置表里查询相应的范围
			{
                if (ControllerRegPtr->CANFD_CTLR0.B.TCSTA == 0x01 && \
                    ControllerRegPtr->CANFD_CTLR0.B.TXBSTA == 0x01)
				{
                    if((MAIL_UNDEFINED == Can_m_FdGetMailBoxState(base, mb_idx)) \
                    || (MAIL_TRANSMIT_SUCCESS == Can_m_FdGetMailBoxState(base, mb_idx)))
                    {
