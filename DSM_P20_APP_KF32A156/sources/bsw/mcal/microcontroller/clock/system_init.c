 ...
 */
    ...
    /* Clear the HSFSEN bit in OSC_HFOSCCAL1 register */
    SFR_CLR_BIT_ASM(OSC_HFOSCCAL1, OSC_HFOSCCAL1_HLSW_POS);
    SFR_SET_BIT_ASM(OSC_HFOSCCAL1, OSC_HFOSCCAL1_SELGAIN1_POS);
    SFR_CLR_BIT_ASM(OSC_HFOSCCAL1, OSC_HFOSCCAL1_SELGAIN0_POS);
    
    /* External oscillator default working  in fast startup mode */
    StartUpCounter = HSE_FASTSTARTUP_TIMEOUT;
    while (--StartUpCounter)
        ;
    GPIO_Write_Mode_Bits(GPIOH_SFR, GPIO_PIN_MASK_11, GPIO_MODE_IN);
    /* Must be wait till EXTHF is ready and if Time out is reached exit */
    OSC_EXTHF_Software_Enable(TRUE);
    OSC_EXTHF_Start_Delay_Config(EXT_START_DELAY_65536);
    ...
    if ((SCLK_Value >= 25) && (SCLK_Value <= 120))
    {
        ...
        {
            volatile uint32_t IntState;
            volatile uint32_t PM_CAL0_TMP, PM_CAL0_HLVL;
            ...
            PM_CAL0_TMP = PM_CAL0;
            PM_CAL0_HLVL = ((PM_CAL0 & 0x3E0) + (3<<5));
            if(PM_CAL0_HLVL > 0x3E0 ) 
                PM_CAL0_HLVL = 0x3E0;
            PM_CAL0_HLVL = PM_CAL0_HLVL | (PM_CAL0 & (~0x3E0));
            PM_CAL0 = PM_CAL0_HLVL;
            asm("nop");
            asm("nop");
            asm("nop");
            asm("nop");

            OSC_SCK_Division_Config(SCLK_DIVISION_4);
            asm("MOV R0,#8"); 	// 4M base 2us wait
            asm("WAIT_LOOP1:");
            asm("SUB R0, R0, #1");
            asm("CMP R0, #0");
            asm("JNZ WAIT_LOOP1");
            OSC_SCK_Source_Config(SCLK_SOURCE_PLL);
            asm("MOV R0,#60"); 	// 30M base 2us wait
            asm("WAIT_LOOP2:");
            asm("SUB R0, R0, #1");
            asm("CMP R0, #0");
            asm("JNZ WAIT_LOOP2");
            OSC_SCK_Division_Config(SCLK_DIVISION_2);
            asm("MOV R0,#120"); 	// 60M base 2us wait
            asm("WAIT_LOOP3:");
            asm("SUB R0, R0, #1");
            asm("CMP R0, #0");
            asm("JNZ WAIT_LOOP3");
            OSC_SCK_Division_Config(SCLK_DIVISION_1);
            asm("MOV R0,#240"); 	// 120M base 2us wait
            asm("WAIT_LOOP4:");
            asm("SUB R0, R0, #1");
            asm("CMP R0, #0");
            asm("JNZ WAIT_LOOP4");
...
void systick_delay_init(uint32_t InputClock)
{
    ...
    SFR_CLR_BIT_ASM(ST_CALI, ST_CALI_STCLKREF_POS);
    fac_us = InputClock >> 1;
    fac_ms = fac_us * 1000;
}
