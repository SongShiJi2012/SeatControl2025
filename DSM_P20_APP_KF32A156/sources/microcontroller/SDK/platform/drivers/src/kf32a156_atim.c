...
void ECCP_PWM_Configuration (ECCP_SFRmap* ECCPx,ECCP_PWMInitTypeDef* eccpInitStruct)
{
    ...
    *(volatile uint32_t*)tmpreg1
    ...
    ECCPx->PXUDCTL =
      SFR_Config(ECCPx->PXUDCTL, ~ECCP_PXUDCTL_PXMPEN, eccpInitStruct->m_PhaseMove << ECCP_PXUDCTL_PXMPEN_POS);
...
/**
  ...
  *                         ECCP_CHANNEL_CMP_ACTIVE: 比较器输出高电平
  *                         ECCP_CHANNEL_BKIN_INACTIVE: ECCP_BKIN引脚上的低电平
  *                         ECCP_CHANNEL_CMP_BKIN_BOTH: ECCP_BKIN引脚上的低电平和比较器输出高电平
...
void ECCP_Channel_Pin_Ctl (ECCP_SFRmap* ECCPx, uint32_t Channel,uint32_t Port, uint32_t ChannelPinCtl)
{
    ...
    ECCPx->PXASCTL1 = SFR_Config (ECCPx->PXASCTL1,
                          ~(ECCP_PXASCTL1_PXSS1L << (Port + Channel * 4)),
                          tmpreg);
}
...
/**
  ...
  *              ECCP_CHANNEL_4: 通道4
  *      ClkDiv：
  *      		 ECCP_CLKDIV1： 分频比为 1:1
  *      		 ECCP_CLKDIV2： 分频比为 1:2
  *      		 ECCP_CLKDIV3： 分频比为 1:3
  *      		 ... ...
...
void ECCP_Filter_CLKDIV_Config(ECCP_SFRmap* ECCPx,uint32_t Channel, uint32_t ClkDiv)
{
    ...
    tmpreg = ClkDiv << ECCP_FAUCTL_FLTP0_POS;
    switch (Channel)
    {
    case ECCP_CHANNEL_1:
