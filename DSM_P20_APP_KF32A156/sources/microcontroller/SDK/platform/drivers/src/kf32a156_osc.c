...
void OSC_SCLK_Configuration (OSC_InitTypeDef* oscInitStruct)
{
    ...
    case SCLK_SOURCE_PLL:
    	...
    	/* 配置PLL启动延时 */
        OSC_CTL2 = SFR_Config(OSC_CTL2, ~OSC_CTL2_PLLDLY, (oscInitStruct->m_StartDelay) << OSC_CTL2_PLLDLY0_POS);
        break;
    default:break;
    }
    /* SCK输入时钟源及分频配置 */
...
/**
  * 描述  配置PLL时钟源分频选择。
  * 输入  PLLDivision: PLL时钟源分频选择，取值范围为：
  *                         PLL_SOURCE_DIV_1: 1/1分频,
  *                         PLL_SOURCE_DIV_2: 1/2分频,
  * 返回  无。
  */
void
OSC_PLL_Division_Config (uint32_t PLLDivision)
{
    uint32_t tmpreg = 0;

    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_PLL_DIVISION(PLLDivision));

    /*---------------- 设置OSC_CTL0寄存器PLLDIV位 ----------------*/
    tmpreg = PLLDivision << OSC_CTL0_PLLDIV_POS;
    OSC_CTL0 = SFR_Config (OSC_CTL0, ~OSC_CTL0_PLLDIV, tmpreg);
}
...
/**
  * 描述  OSC中断使能。
  * 输入  InterruptType: OSC中断类型，取值为：
  * 							OSC_INT_INTLF：内部低频中断
  * 							OSC_INT_INTHF：内部高频中断
  * 							OSC_INT_EXTLF：外部低频中断
  * 							OSC_INT_EXTHF：外部高频中断
  * 							OSC_INT_PLL：    PLL中断
  * 							OSC_INT_LP4M：  LP4M中断
  *  	NewState: 内部低频中断使能状态，取值范围为：TRUE 或 FALSE。
  * 返回  无。
  * 备注：与单时钟使能函数功能相同。
  */
void OSC_INT_Enable(uint32_t InterruptType, FunctionalState NewState)
{
	...
	/* 参数校验 */
	CHECK_RESTRICTION(CHECK_OSC_INT_ENABLE(InterruptType));
	CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));

	tmpreg = (uint32_t)1 << (InterruptType + OSC_INT_ILFIE_POS);
	if (NewState != FALSE)
	{
		/* 中断使能 */
		OSC_INT |= tmpreg;
...
/**
  ...
  * 							OSC_INT_PLL：    PLL中断
  * 							OSC_INT_LP4M：  LP4M中断
  * 							OSC_INT_CKF：    时钟故障检测中断
  * 返回  中断状态，0：未发生中断，1：发生中断。
  * 备注：与获取单时钟中断标志位函数功能相同。
  */
FlagStatus OSC_Get_INT_Flag(uint32_t InterruptType)
{
	...
	/* 参数校验 */
	CHECK_RESTRICTION(CHECK_OSC_INT_FLAG(InterruptType));

	tmpreg = (uint32_t)1 << InterruptType;
    if (OSC_INT & tmpreg)
    {
        return SET;
    }
...
/**
  ...
  */
void
OSC_Clear_INT_Flag(uint32_t InterruptType)
{
	uint32_t tmpreg = 0;
