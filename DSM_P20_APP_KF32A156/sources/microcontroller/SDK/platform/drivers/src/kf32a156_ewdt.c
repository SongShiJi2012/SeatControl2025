...
/**
  ...
  * 返回  无。
  */
void EWDT_Deinit (void)
{
    /* 复位EWDT模块 */
    RST_CTL3_Peripheral_Reset_Enable(RST_CTL3_EWDTRST, TRUE);
    RST_CTL3_Peripheral_Reset_Enable(RST_CTL3_EWDTRST, FALSE);
    /* 使能EWDT时钟 */
    PCLK_CTL3_Peripheral_Clock_Enable(PCLK_CTL3_EWDTCLKEN, FALSE);
}
...
{
	volatile uint32_t wait_flag = 0x0000;
	SFR_SET_BIT_ASM(EWDT_CTL, EWDT_CTL_WT40IC_POS);
	while((EWDT_CTL & EWDT_CTL_WT40IF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
	SFR_CLR_BIT_ASM(EWDT_CTL, EWDT_CTL_WT40IC_POS);
	if(wait_flag==FLAG_TIMEOUT)
	{
		return FAILURE;
	}
