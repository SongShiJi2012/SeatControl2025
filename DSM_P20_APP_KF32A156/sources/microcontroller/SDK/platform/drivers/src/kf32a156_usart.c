...
/**
  ...
  * 返回  无。
  */
void USART_Deinit (USART_SFRmap* USARTx)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    ...
    if (USARTx == USART0_SFR)
    {
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART0RST, TRUE);
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART0RST, FALSE);
        PCLK_CTL1_Peripheral_Clock_Enable(PCLK_CTL1_USART0CLKEN, FALSE);
    }
    else if (USARTx == USART1_SFR)
    {
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART1RST, TRUE);
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART1RST, FALSE);
        PCLK_CTL1_Peripheral_Clock_Enable(PCLK_CTL1_USART1CLKEN, FALSE);
    }
    else if (USARTx == USART2_SFR)
    {
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART2RST, TRUE);
        RST_CTL1_Peripheral_Reset_Enable(RST_CTL1_USART2RST, FALSE);
        PCLK_CTL1_Peripheral_Clock_Enable(PCLK_CTL1_USART2CLKEN, FALSE);
    }
    else if (USARTx == USART5_SFR)
    {
        RST_CTL2_Peripheral_Reset_Enable(RST_CTL2_USART5RST, TRUE);
        RST_CTL2_Peripheral_Reset_Enable(RST_CTL2_USART5RST, FALSE);
        PCLK_CTL2_Peripheral_Clock_Enable(PCLK_CTL2_USART5CLKEN, FALSE);
    }
    else
    {
        ;
...
void USART_Configuration (USART_SFRmap* USARTx, USART_InitTypeDef* usartInitStruct)
{
    ...
    if(0U != (uint32_t)usartInitStruct->m_BaudRateDenominator)
    {
    ...
    else
    {
        tmpreg = ((uint32_t)usartInitStruct->m_BaudRateInteger << USART_BRGR_BRGM0_POS)  \
                | ((uint32_t)usartInitStruct->m_BaudRateNumerator << USART_BRGR_BRGF1_0_POS)   \
                | ((uint32_t)1U << USART_BRGR_BRGF2_0_POS);
    }
...
/**
  ...
  *                   USART_CPOL_RISE:数据在时钟上升沿同步，
  *                   USART_CPOL_FALL :数据在时钟下降沿同步。
  * 返回  无。
  */
...
void USART_BaudRate_Decimal2_Config(USART_SFRmap* USARTx, uint32_t DIV)
{
    ...
    {
        tmpreg = 1U << USART_BRGR_BRGF2_0_POS ;
    }
...
{
	volatile uint32_t wait_flag=0x0000,wait_flag1=0x0000;
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));

    /*---------------- 设置USART_TBUFR寄存器 ----------------*/
    USARTx->TBUFR = Data;
    while(((USARTx->STR & USART_STR_TFEIF1))&& (wait_flag1 != FLAG_TIMEOUT))
	{
		wait_flag1++;
	}
...
FlagStatus USART_Get_Transmit_BUFR_Empty_Flag (USART_SFRmap* USARTx)
{
    ...
    if (USARTx->STR & USART_STR_TFEIF1)
    {
        /* USART 发送BUF为空*/
        return SET;
    }
...
RetStatus USART_Clear_Transmit_BUFR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((!((USARTx->STR & USART_STR_TFEIF1)>>USART_STR_TFEIF1_POS))&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
