...
/* 
 * @brief GPIO_Set_Output_Data_Bits_ASM, 带临界保护的GPIO输出函数.

 * @attention GpioPin, 不可传入变量, 只能传入枚举/宏定义等立即数
              使用时, 操作同一组GPIO引脚时, 不可与其他操作POR寄存器的接口混用, 否则会失去临界保护功能.
              (如:GPIO_Set_Output_Data_Bits_ASM与GPIO_Set_Output_Data_Bits,GPIO_Toggle_Output_Data_Config 混用)
 * @param GPIOx 指向GPIO寄存器映射的指针
 * @param GpioPin GPIO引脚编号，取值范围为GPIO_Pin_Num_0到GPIO_Pin_Num_15
 * @param BitsValue 0表示Bit_RESET，1表示Bit_SET
*/
#define GPIO_Set_Output_Data_Bits_ASM(GPIOx, GpioPin, BitsValue) \
do{\
  if((BitsValue) > 0U)\
  {\
    SFR_SET_BIT_ASM((GPIOx)->POR, (GpioPin));\
  }\
  else if(0U == (BitsValue))\
  {\
    SFR_CLR_BIT_ASM((GPIOx)->POR, (GpioPin));\
  }\
  else\
  {}\
}while(0)
