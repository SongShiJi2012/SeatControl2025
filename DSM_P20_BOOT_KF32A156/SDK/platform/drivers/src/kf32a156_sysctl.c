...
/**
  * 描述 内核 DEBUG 模式断点与单步模式控制。
  * 输入  Mode:	SYSCTL_IGNORE_BREAKPOINT : 单步模式下忽略断点响应
  * 			SYSCTL_SESPONSE_BREAKPOINT : 单步模式下与断点同时响应
  * 返回  无。
  */
void SYSCTL_Debug_BreakPoint_SingleStep_Config(uint32_t Mode)
{
	uint32_t tmpreg = 0x00;

    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_SYSCTL_SINGLESTEP_BREAKPOINT(Mode));

    tmpreg = SYS_ARCTL_WRITE_CTL | ((uint32_t)Mode << SYS_ARCTL_CDBUGBS_POS);
    SYS_ARCTL = SFR_Config(SYS_ARCTL, ~(SYS_ARCTL_CDBUGBS | SYS_ARCTL_ARKEY), tmpreg);
}
...
/**
  * 描述 POP PC 打断控制。
  * 输入  NewState:取值为：	TRUE :POP PC 不会被中断打断
  * 						FALSE:POP PC 会被中断打断
  * 返回  无。
  */
...
//     tmpreg = SYS_ARCTL_WRITE_CTL | ((uint32_t)NewState << SYS_ARCTL_CPOPINT_POS);
//     SYS_ARCTL = SFR_Config(SYS_ARCTL, ~(SYS_ARCTL_CPOPINT | SYS_ARCTL_ARKEY), tmpreg);
// }

/**
  * 描述 除 0 标志位产生控制。
  * 输入  Select: 	SYSCTL_DIV0_FLAG_LATER：除 0 标志位在除法计算之后产生
  * 				SYSCTL_DIV0_FLAG_PROCESS：除 0 标志位在除法计算过程中产生
  * 返回  无。
  */
...
//     /* 参数校验 */
//     CHECK_RESTRICTION(CHECK_SYSCTL_DIV0_FLAG_SELECT(Select));

//     tmpreg = SYS_ARCTL_WRITE_CTL | ((uint32_t)Select << SYS_ARCTL_CDIV_POS);
//     SYS_ARCTL = SFR_Config(SYS_ARCTL, ~(SYS_ARCTL_CDIV | SYS_ARCTL_ARKEY), tmpreg);
// }

/**
  * 描述 高效模式下 DMA 冲突读写控制位。
  * 输入  Select: 	SYSCTL_EFFICIENT_CONFLICT_FAIL：高效模式下 DMA 与 CPU 冲突会出现 DMA 操作失败
  * 				SYSCTL_EFFICIENT_CONFLICT_ABNORMAL：高效模式下 DMA 与 CPU 冲突不会出现 DMA 操作异常
  * 返回  无。
  */
// void
// SYSCTL_Efficient_Conflict_Config(uint32_t Select)
// {
// 	uint32_t tmpreg = 0x00;

//     /* 参数校验 */
//     CHECK_RESTRICTION(CHECK_SYSCTL_EFFICIENT_CONFLICT_SELECT(Select));

//     tmpreg = SYS_ARCTL_WRITE_CTL | ((uint32_t)Select << SYS_ARCTL_CDMAHEC_POS);
//     SYS_ARCTL = SFR_Config(SYS_ARCTL, ~(SYS_ARCTL_CDMAHEC | SYS_ARCTL_ARKEY), tmpreg);
// }
