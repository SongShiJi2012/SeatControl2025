...
//	SFR_SET_BIT_ASM(OSC_CTL0, OSC_CTL0_PMWREN_POS);//在WDOG初始化打开
//	SFR_SET_BIT_ASM(PM_CAL0, PM_CTL0_VDDPORON_POS);
	...
	CANFD_PTR[Can_Controller_Index]->CANFD_CTLR2.B.AMCKDIV = Clock_Divide_4;//仲裁时钟源如果是120M,就要用4分频
...
#endif
	/* Enable CANFD Mode */
	CANFD_PTR[Can_Controller_Index]->CANFD_CTLR0.B.CANFDEN = Can_ControllerConfig->EnableFD;//如果是传参数方法要主要传过来的参数设置
	...
	while (delay_count--)
		;
	CANFD_PTR[Can_Controller_Index]->CANFD_CTLR0.B.TXR = STD_ON;
//	SFR_CLR_BIT_ASM(OSC_CTL0, OSC_CTL0_PMWREN_POS);//在WDOG初始化打开
}
