...
static INT8U Srvl_E2DataCompare(INT32U lu32Dest, INT32U lu32Size, const INT8U* lpu8DataBuf)
{
	...
	//ecual_I2C_read(EEPROM_I2C,lu32Dest - e2Config.EERAMBase,Databuf,lu32Size);
    Srvl_Eeprom_Read(lu32Dest - e2Config.EERAMBase,lu32Size,Databuf);
...
void Srvl_E2Read(INT32U lu32Dest, INT32U lu32Size, INT8U *lpu8ReadBuf)
{
    ...
    Srvl_Eeprom_Read(lu32Dest - e2Config.EERAMBase,lu32Size,lpu8ReadBuf);
}
...
INT8U Srvl_E2Write(INT32U lu32Dest, INT32U lu32Size, const INT8U * lpu8DataBuf)
{
	...
	{
		len_Ret = Srvl_Eeprom_Write(lu32Dest - e2Config.EERAMBase,lu32Size,lpu8DataBuf);
	}
	else
	{
		lu8Ret = D_ParaErr;
	...
	if(len_Ret == STATUS_BUSY)
	{
		lu8Ret = D_E2_WriteWait;
	}
	else if(len_Ret == STATUS_SUCCESS)
	{
		lu8Ret = D_Success;
	}
	else
...
static void Srvl_E2ReadTask(void)
{
	...
	if((E2PROMOpInfo.DataNum) < EEPROMBUFFMAXNUM)
	{
        ...
        ResultSts = Srvl_Eeprom_Read(E2PROMOpInfo.E2pDataAddress,E2PROMOpInfo.DataNum,gu8EepromReadBuffer);
...
void Srvl_RestoreDataFromBackUpID(INT16U DataBlockId)
{
	...
	if(Srvl_GetE2DataBlockBackUpID(DataBlockId) != NOBACKUP)
	{
...
static void Srvl_E2OPEndCallBack(eE2promOpType EEOpType, E2PROMOperateResult EEOpRes)
{
	...
	INT16U BackUpID = NOBACKUP;//当前ID的备份ID
	...
	else if(EEOpType == EEOpRD)
	{
		...
		{
			//READ RIGHT
			if(OriginalIndex != NOBACKUP)
			{
				Srvl_RestoreDataFromBackUpID(OriginalIndex);
				OriginalIndex = NOBACKUP;
			}
			Srvl_SetE2DataBlockOptResult(CurUpID, E2promOpOK);
		}
		else if(NOBACKUP != BackUpID)
		{
			//如果有备份就去读备份
 ...
 ***********************************************************************************************/
void Srvl_WriteMemIndexData(enSrvMemIndex MemIndex, INT8U * ptru8data, INT8U len)
{
	INT8U i = 0;
	INT8U DataLen = 0;
	INT8U StartOffset = 0;
